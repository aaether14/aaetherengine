...
class ringbuffer
{
	...
	public:
	ringbuffer(aae_size_t size, AAEAllocatorInterface& allocator) : 
		m_buffer(nullptr),
		m_head(0), 
		m_tail(0),
	...
	virtual ~ringbuffer()
	{
		if (m_buffer) AAE_DELETE_ARRAY(m_buffer, m_allocator);
	}
	inline aae_size_t get_size() const { return m_size; }
	...
	{
		if (empty()) { if(underflow()) return pop(c_data); return false; } /** handle the underflow and try to pop again **/
	...
	inline i32 get_tail() const { return m_tail; }
	inline i32 get_head() const { return m_head; }
	inline void set_tail(i32 tail) { m_tail = tail % m_size; }
	inline void set_head(i32 head) { m_head = head % m_size; }
