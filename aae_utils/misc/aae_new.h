...
extern "C++" inline void* operator new(aae_size_t, void* ptr) { return ptr; }
#endif
...
{
	void* address = reinterpret_cast<void*>(reinterpret_cast<aae_size_t>(array) - sizeof(aae_size_t));
...
static inline T* aae_new_array(Arena& arena, aae_size_t n_instances)
{
	...
	*(static_cast<aae_size_t*>(address)) = n_instances;
	address = reinterpret_cast<void*>(reinterpret_cast<aae_size_t>(address) + sizeof(aae_size_t));
...
static inline void* aae_aligned_new(aae_size_t size, aae_size_t alignment, Arena& arena) /** aligned to a power of 2 **/
{
	...
	*(reinterpret_cast<void**>(reinterpret_cast<aae_size_t>(aligned_address) - sizeof(void*))) = address;
...
{
	arena.Deallocate(*(reinterpret_cast<void**>(reinterpret_cast<aae_size_t>(aligned_address) - sizeof(void*))));
}
